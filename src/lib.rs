...
use enigo::{
    Axis,Button, Coordinate, Enigo, Key, Keyboard, Mouse, Settings,
...
pub fn get_mouse_position(&mut self) -> Result<Point, napi::Error> {
    let (x, y) = self.enigo.location().map_err(|e| napi::Error::from_reason(e.to_string()))?;
    Ok(Point::new(x, y))
}

#[napi]
pub fn set_mouse_position(&mut self, x: i32, y: i32) -> Result<(), napi::Error> {
    self.enigo.move_mouse(x, y, Coordinate::Abs).map_err(|e| napi::Error::from_reason(e.to_string()))
}
...
pub fn set_button_click(&mut self, button: MouseButton) -> Result<(), napi::Error> {
    ...
    self.enigo.button(button, Click).map_err(|e| napi::Error::from_reason(e.to_string()))
}
...
pub fn set_button_toggle(&mut self, button: MouseButton, down: bool) -> Result<(), napi::Error> {
    ...
    self.enigo.button(button, direction).map_err(|e| napi::Error::from_reason(e.to_string()))
}

#[napi]
pub fn set_mouse_scroll(&mut self, direction: ScrollDirection, clicks: i32) -> Result<(), napi::Error> {
...
pub fn type_text(&mut self, content: String) -> Result<(), napi::Error> {
    self.enigo.text(content.as_str()).map_err(|e| napi::Error::from_reason(e.to_string()))
}
...
pub fn press_key(&mut self, keys: Vec<KeyboardKey>) -> Result<(), napi::Error> {
    for key in keys {
        ...
        self.enigo.key(key, Click).map_err(|e| napi::Error::from_reason(e.to_string()))?;
    }
    Ok(())
}
...
pub fn release_key(&mut self, keys: Vec<KeyboardKey>) -> Result<(), napi::Error> {
    for key in keys {
        let key = transform_key(key); // Fix: use snake_case for function name
        self.enigo.key(key, Release).map_err(|e| napi::Error::from_reason(e.to_string()))?;
    }
    Ok(())
}
}
